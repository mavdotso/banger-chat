...
export default function LoginForm() {
    ...
    return (
        <div>
            <span className="text-white font-bold select-none">Log in or create a new account</span>
            <Form {...form}>
                <form className="w-full flex flex-col py-4 gap-1.5 text-start" onSubmit={(...args) => void form.handleSubmit(onSubmit)(...args)}>
                    <FormField
                    ...
                    />
                    <Button type="submit" disabled={isPending} className="h-14 rounded-xl my-1 font text-base font-semibold bg-white hover:bg-white text-black">
                        {isPending ? <Loader className="h-10 w-10" aria-hidden="true" /> : 'Log in'}
