import { IMessage } from '@/store/channel-store';
...
function ChatBubbleAvatar({ isMember, message }: ChatBubbleAvatarProps) {
    return (
        <Avatar className="overflow-visible relative">
            {isMember && <div className="absolute top-0 right-0 w-2 h-2 bg-green-500 rounded-full border-2 border-foreground" />}
